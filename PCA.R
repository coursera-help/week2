library(calibrate)
library(FactoMineR)
setwd("C://Users//tejas//Desktop")
data_iris<- read.csv("stud.csv")
head(data_iris)
colSums(is.na(data_iris))
data_iris_1<- data_iris[,-1]
...
cov <- cov(data_iris_1)
eigen <- eigen(cov)
rownames(eigen$vectors) <- c("anx","agora","arach","adven","extra","social")
colnames(eigen$vectors) <- c("PC1","PC2","PC3","PC4","PC5","PC6")
eigen

# Note that the sum of the eigen values equals the total variance of the data
sum(eigen$values)
var(data_iris_1[,1]) + var(data_iris_1[,2])+ var(data_iris_1[,3])+ var(data_iris_1[,4])+ var(data_iris_1[,5])+ var(data_iris_1[,6])
